...
class AuthController extends Controller
{
    ...
    {
        if(!$this->isPin($request)) {
            try {

                $credentials = $this->getUsingPin($request);
                return $this->attemptCredentials($credentials);

            } catch (\Exception $exception) {

                return response()
                    ->json(['error' => $exception->getMessage()], Response::HTTP_NOT_FOUND);
            }
     ...
     * @return mixed
     */
    private function isPin($request)
    {
     ...
     * @return mixed
     */
    private function getUsingPin($request)
    {
        try {
            $user = User::select('email')->wherePinCode($request->email)->firstOrFail();
            $credentials = [
                'email' => $user->email,
